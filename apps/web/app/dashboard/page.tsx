...
export default function DashboardPage() {
  ...
  const load = useCallback(async (id: string) => {
    ...
    setError(null);
    try {
      const [u, p] = await Promise.all([api.getUser(id), api.getPosts(id)]);
      setUser(u);
      setPosts(p);
    } catch (e) {
        ...
        <main className="px-6 md:px-12 lg:px-16 py-12 lg:py-16">
          ...
          {posts.length === 0 ? (
            ...
            <ul className="space-y-px">
              {posts.map((post, i) => {
                const status =
                  STATUS_TOKENS[post.status] ?? STATUS_TOKENS.pending;
                return (
                  <li
                    key={post.id}
                  ...
                  >
                    <div className="flex items-center gap-3 mb-4 text-[11px] font-mono">
                      ...
                      />
                      <span
                        className={`uppercase tracking-widest ${status.text}`}
                      >
                        {status.label}
                      </span>
